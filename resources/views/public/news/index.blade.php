            ...
            <form method="GET" action="{{ route('news.index') }}" class="mt-5 sm:mt-6 flex flex-wrap gap-2 sm:gap-3">
                <div class="relative w-full sm:w-auto">
                    ...
                    <input name="q" value="{{ request('q') }}" placeholder="Search news…"
                           class="w-full sm:w-60 rounded-full border-2 border-white/30 bg-white/10 text-white placeholder-white/50
                                  pl-9 pr-4 py-2 text-xs sm:text-sm focus:border-white focus:outline-none backdrop-blur-sm" />
                </div>

                <div class="flex gap-2 sm:gap-3 flex-wrap">
                    <select name="category"
                            class="rounded-full border-2 border-white/30 bg-white/10 text-white px-3 sm:px-4 py-2 text-xs sm:text-sm focus:border-white focus:outline-none">
                    ...
                    <button class="rounded-full border-2 border-white bg-white px-4 sm:px-5 py-2 text-xs sm:text-sm font-bold text-tpc-primary hover:bg-tpc-secondary hover:border-tpc-secondary hover:text-white transition">
                        Filter
                    </button>

                    @if(request('q') || request('category'))
                        <a href="{{ route('news.index') }}"
                           class="inline-flex items-center gap-1 rounded-full border-2 border-white/40 px-4 sm:px-5 py-2 text-xs sm:text-sm font-bold text-white/80 hover:text-white hover:border-white transition">
        ...
        <div class="max-w-7xl mx-auto px-4 py-10 sm:py-14">
            ...
            @if($posts->isNotEmpty())
                ...
                <a href="{{ route('news.show', $featured) }}"
                   class="news-featured-card group relative block bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-5 sm:mb-8
                    ...
                    <div class="sm:flex min-h-[220px] sm:min-h-[280px]">
                        ...
                        <div class="flex-1 flex flex-col justify-between p-4 sm:p-6 lg:p-8">
                            <div>
                                <div class="flex flex-wrap items-center gap-2 mb-3">
                                    ...
                                    @if($featured->published_at)
                                        <time class="text-[11px] sm:text-xs text-gray-400 font-medium sm:ml-auto" datetime="{{ $featured->published_at->toDateString() }}">
                                            {{ $featured->published_at->format('M d, Y') }}
                                        </time>
                                    @endif
                                </div>

                                <h2 class="text-base sm:text-xl lg:text-2xl font-bold text-gray-900 group-hover:text-tpc-primary transition-colors duration-200 leading-snug mb-2 sm:mb-3">
                                    {{ $featured->title }}
                                </h2>
                                <p class="text-xs sm:text-sm text-gray-500 leading-relaxed line-clamp-2 sm:line-clamp-3">
                            ...
                            <div class="mt-4 sm:mt-6 flex items-center justify-between">
                                <span class="inline-flex items-center gap-1.5 text-xs sm:text-sm font-bold text-tpc-primary group-hover:gap-3 transition-all duration-200">
                                    Read Article
                        ...
                        <article class="news-card group bg-white rounded-2xl border border-gray-200 shadow-sm
                            ...
                            <div class="p-4 sm:p-5 flex flex-col flex-1">
                                @if($post->published_at)
                                    <time class="text-[10px] sm:text-[11px] text-gray-400 font-medium mb-2 block tabular-nums"
                                          datetime="{{ $post->published_at->toDateString() }}">
                                        {{ $post->published_at->format('M d, Y') }}
                                    </time>
                                @endif

                                <h2 class="text-xs sm:text-sm font-bold text-gray-800 group-hover:text-tpc-primary transition-colors duration-200 leading-snug flex-1 line-clamp-3">
            ...
            @else
                <div class="py-16 sm:py-24 flex flex-col items-center justify-center text-center
                    ...
                    <div class="h-12 w-12 sm:h-14 sm:w-14 rounded-full bg-gray-100 flex items-center justify-center mb-4">
                        <svg class="h-5 w-5 sm:h-6 sm:w-6 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10l6 6v8a2 2 0 01-2 2z"/>
                        </svg>
                    </div>
                    ...
                    @if(request('q') || request('category'))
                        <a href="{{ route('news.index') }}"
                           class="mt-4 inline-flex items-center gap-1 text-xs sm:text-sm font-bold text-tpc-primary hover:underline">
                            ← Clear filters
                        </a>
                    @endif
                </div>
