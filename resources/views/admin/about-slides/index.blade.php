...
<div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between mb-8">
    <div>
        <div class="flex items-center gap-2 mb-1">
            ...
            <h1 class="text-xl font-semibold text-neo-ink tracking-tight">About Slides</h1>
        </div>
        <p class="text-sm text-neo-ink/50 pl-3">Manage the image carousel shown in the About section on the homepage.</p>
    </div>
...
@if($slides->isEmpty())
    ...
    <div class="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p class="text-sm font-semibold text-neo-ink/70 mb-1">No slides yet</p>
        <p class="text-xs text-neo-ink/40 mb-5">Add your first slide to populate the homepage carousel.</p>
        <a href="{{ route('admin.about-slides.create') }}"
...
@else
    ...
    <div class="inline-flex items-center gap-4 mb-5 rounded-xl bg-neo-surface shadow-neo-sm px-4 py-2 text-xs text-neo-ink/50">
        <span><span class="font-bold text-neo-ink">{{ $totalCount }}</span> total</span>
        <span class="h-3 w-px bg-black/10"></span>
        <span><span class="font-bold text-tpc-secondary">{{ $activeCount }}</span> active</span>
        <span class="h-3 w-px bg-black/10"></span>
        <span><span class="font-bold text-neo-ink/40">{{ $totalCount - $activeCount }}</span> hidden</span>
    </div>
    ...
    <div class="flex flex-col gap-3 sm:hidden">
        @foreach($slides as $slide)
            <div class="rounded-2xl bg-neo-surface shadow-neo overflow-hidden"
                ...
                {{-- Image strip --}}
                <div class="relative h-32 w-full bg-neo-bg overflow-hidden">
    ...
    <div class="hidden sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        @foreach($slides as $slide)
            <div class="group relative rounded-2xl bg-neo-surface shadow-neo overflow-hidden transition-all duration-200 hover:shadow-neo-hover">

                {{-- Image --}}
                <div class="relative h-44 w-full bg-neo-bg overflow-hidden">
                    <img src="{{ asset('storage/' . $slide->image_path) }}"
                         alt="{{ $slide->title ?: 'Slide ' . $slide->sort_order }}"
                ...
                {{-- Actions --}}
                <div class="flex items-center justify-between px-4 py-3 border-t border-black/[0.06]">
                    <a href="{{ route('admin.about-slides.edit', $slide) }}"
        ...
        {{-- Add new card --}}
        <a href="{{ route('admin.about-slides.create') }}"
           class="flex flex-col items-center justify-center gap-3 h-full min-h-[220px] rounded-2xl bg-neo-bg shadow-neo-inset-sm text-neo-ink/40 hover:shadow-neo-inset hover:text-tpc-primary transition-all duration-200 group">
            <div class="h-10 w-10 rounded-xl bg-neo-surface shadow-neo-sm flex items-center justify-center group-hover:scale-110 transition-transform">
                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/>
                </svg>
            </div>
            <span class="text-sm font-semibold">Add Slide</span>
        </a>
    </div>
