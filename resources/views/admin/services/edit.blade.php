...
@section('content')
    <form action="{{ route('admin.services.update', $service) }}" method="POST" enctype="multipart/form-data" class="space-y-6 max-w-2xl">
        @csrf @method('PATCH')
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $service->title) }}" required
                   class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-tpc-primary focus:outline-none focus:ring-2 focus:ring-tpc-primary/20 @error('title') border-red-300 @enderror">
            @error('title')
        ...
        <div>
            ...
            <textarea name="description" rows="3"
                      class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-tpc-primary focus:outline-none focus:ring-2 focus:ring-tpc-primary/20 resize-none">{{ old('description', $service->description) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-xs font-bold text-gray-600 mb-1.5">Display Order</label>
            ...
            <div>
                <label class="block text-xs font-bold text-gray-600 mb-1.5">Visibility</label>
                <label class="mt-2.5 flex items-center gap-2 cursor-pointer">
