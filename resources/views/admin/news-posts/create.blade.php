...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
            ...
            <h1 class="text-lg font-semibold text-neo-ink">Create News Post</h1>
        </div>
    </div>
    ...
    @if(!auth()->user()->is_super_admin)
        <div class="mb-5 flex items-start gap-3 rounded-2xl bg-neo-bg shadow-neo-inset-sm p-4">
            ...
            <p class="text-xs">
                <span class="font-semibold text-amber-700">Heads up:</span>
                <span class="text-neo-ink/70">After saving, this post will be sent to the superadmin for review before it goes live.</span>
            </p>
        </div>
    @endif
        ...
        <form method="POST" action="{{ route('admin.news-posts.store') }}" enctype="multipart/form-data"
            ...
            <div>
                ...
                <textarea name="body" id="body" rows="10" required
                          ...
                          class="w-full rounded-xl bg-neo-bg shadow-neo-inset-sm border-0 px-3 py-2.5 text-sm text-neo-ink placeholder-neo-ink/30 focus:outline-none focus:ring-2 focus:ring-tpc-primary/30 transition resize-none">{{ old('body') }}</textarea>
            ...
            <div>
                <label for="photos-input" class="block text-sm font-medium text-neo-ink/70 mb-1.5">
                    Photos <span class="text-neo-ink/35 font-normal">(optional · up to 20)</span>
                </label>

                {{-- Drop zone --}}
                <div id="drop-zone"
                     class="relative flex flex-col items-center justify-center gap-2 rounded-xl bg-neo-bg shadow-neo-inset-sm px-4 py-8 text-center cursor-pointer transition">
                    ...
                    <p class="text-xs text-neo-ink/35">PNG / JPG / WEBP · max 5 MB each · up to 20 images</p>
