    ...
    <div class="max-w-2xl">
        <form method="POST" action="{{ route('admin.news-posts.update', $newsPost) }}" enctype="multipart/form-data"
            ...
            <div>
                ...
                @if($newsPost->galleryImages->isNotEmpty())
                    <p class="text-xs font-semibold text-tpc-ink/50 mb-2">
                        Current photos ({{ $newsPost->galleryImages->count() }})
                    </p>
                    <div class="grid grid-cols-3 sm:grid-cols-4 gap-2 mb-4">
                        @foreach($newsPost->galleryImages as $img)
                            <div class="relative group rounded-xl overflow-hidden border border-gray-200 aspect-square bg-gray-100">
                                ...
                                <form method="POST"
                                      action="{{ route('admin.news-posts.gallery.destroy', [$newsPost, $img]) }}"
                                      class="absolute inset-0 flex items-start justify-end p-1
                                             opacity-0 group-hover:opacity-100 transition">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                            class="h-6 w-6 rounded-full bg-red-600 text-white text-xs font-bold
                                                   flex items-center justify-center shadow hover:bg-red-700 transition"
                ...
                {{-- Add new photos --}}
                <div id="edit-gallery-preview-grid"
                     class="hidden grid grid-cols-3 sm:grid-cols-4 gap-2 mb-3"></div>

                <label for="edit-gallery-input"
                    ...
                    <span id="edit-gallery-btn-label">Add Photos</span>
                </label>
                <input type="file" name="gallery_images[]" id="edit-gallery-input"
            ...
            <script>
                ...
                (function () {
                    ...
                    const grid     = document.getElementById('edit-gallery-preview-grid');
                    const btnLabel = document.getElementById('edit-gallery-btn-label');
                    ...
                    function renderPreviews() {
                        ...
                        btnLabel.textContent = `Add More (${files.length} queued)`;

                        files.forEach((file, index) => {
                            const reader = new FileReader();
                            reader.onload = (e) => {
                                const wrap = document.createElement('div');
                                wrap.className = 'relative group rounded-xl overflow-hidden border border-tpc-primary/20 aspect-square bg-gray-100';
                                wrap.innerHTML = `
                                    <img src="${e.target.result}" class="w-full h-full object-cover" alt="">
                                    <div class="absolute top-1 left-1 bg-tpc-primary text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">NEW</div>
