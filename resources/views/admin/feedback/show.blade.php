            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div class="min-w-0">
                    ...
                    <div class="flex items-center gap-1 mb-2">
                        @for($i = 1; $i <= 5; $i++)
                            <svg class="h-6 w-6 {{ $i <= $feedback->rating ? 'text-amber-300' : 'text-white/20' }}" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        @endfor
                        <span class="ml-1 text-lg font-bold text-white">{{ $feedback->rating }}/5</span>
                    </div>

                    <div class="mt-1 flex flex-wrap items-center gap-3 text-sm text-white/60">
        ...
        <div class="max-w-4xl mx-auto px-4 py-14">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-6">
                ...
                <div class="px-7 py-5 border-b border-gray-100 flex items-center gap-4">
                    ...
                    <div>
                        <p class="font-bold text-gray-800">{{ $feedback->name ?: 'Anonymous' }}</p>
                        <p class="text-sm text-gray-500">{{ $feedback->email ?: 'No email provided' }}</p>
                    </div>
                    <div class="ml-auto text-right">
                        <p class="text-xs font-semibold text-gray-500">{{ $feedback->created_at->format('M d, Y') }}</p>
                ...
                <div class="px-7 py-7">
                    @if($feedback->message)
                        <div class="whitespace-pre-wrap text-sm text-gray-700 leading-relaxed">{{ $feedback->message }}</div>
                    @else
                        <p class="text-sm text-gray-400 italic">No comment was left with this rating.</p>
                    @endif
                </div>

                @if($feedback->page_url)
                    <div class="px-7 pb-5">
                        <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-1">Submitted from</p>
                        <p class="text-xs text-gray-500 break-all">{{ $feedback->page_url }}</p>
                    </div>
                @endif
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <form method="POST" action="{{ route('admin.feedback.respond', $feedback) }}" class="p-7">
                    ...
                    <textarea name="admin_response" rows="4" placeholder="Log how this was handled, or draft a response here (visible to admins only)..."
                        class="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm text-gray-800 focus:border-tpc-primary focus:ring-0 focus:outline-none transition resize-none placeholder-gray-300">{{ old('admin_response', $feedback->admin_response) }}</textarea>
                    <div class="mt-4 flex justify-end">
                        <button type="submit"
                            class="inline-flex items-center gap-2 rounded-full border-2 border-tpc-primary bg-tpc-primary px-5 py-2.5 text-sm font-bold text-white hover:bg-tpc-secondary hover:border-tpc-secondary transition">
                            Save Note
                        </button>
                    </div>
                </form>
