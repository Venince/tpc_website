        ...
        <div class="max-w-7xl mx-auto px-4 py-10">
            ...
            <div class="flex flex-col gap-2">
                <div class="flex flex-wrap items-end gap-4">
                    <div>
                        <h1 class="text-3xl sm:text-4xl font-bold text-white leading-tight">Feedback</h1>
                        <p class="mt-2 text-sm text-white/70">
                            Unread: <span class="font-bold text-white">{{ $unreadCount }}</span>
                        </p>
                    </div>
                    <div class="flex items-center gap-1.5 bg-white/10 border border-white/20 rounded-xl px-3 py-1.5 backdrop-blur-sm">
                        <svg class="h-4 w-4 text-amber-300" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <span class="text-xs text-white/50">avg</span>
                    </div>
                </div>

                {{-- Filter form --}}
                <form method="GET" class="w-full mt-4 flex flex-col gap-2 sm:flex-row sm:items-center flex-wrap">
                    <input
                        name="q"
                        value="{{ $q }}"
                        placeholder="Search name, email, comment..."
                        class="flex-1 min-w-[180px] rounded-xl border-2 border-white/30 bg-white/10 text-white placeholder-white/50 px-4 py-2.5 text-sm focus:border-white focus:outline-none backdrop-blur-sm"
                    />
                    <select name="status" class="rounded-xl border-2 border-white/30 bg-tpc-primary text-white px-3 py-2.5 text-sm focus:border-white focus:outline-none appearance-none cursor-pointer">
                    ...
                    <select name="rating" class="rounded-xl border-2 border-white/30 bg-tpc-primary text-white px-3 py-2.5 text-sm focus:border-white focus:outline-none appearance-none cursor-pointer">
                        <option value="all" class="text-tpc-ink" @selected($rating === 'all')>All Ratings</option>
                        @for($r = 5; $r >= 1; $r--)
                            <option value="{{ $r }}" class="text-tpc-ink" @selected((string)$rating === (string)$r)>{{ $r }} Star{{ $r > 1 ? 's' : '' }}</option>
                        @endfor
                    </select>
                    <select name="category" class="rounded-xl border-2 border-white/30 bg-tpc-primary text-white px-3 py-2.5 text-sm focus:border-white focus:outline-none appearance-none cursor-pointer">
                        <option value="all" class="text-tpc-ink" @selected($category === 'all')>All Categories</option>
                        @foreach($categories as $key => $label)
                            <option value="{{ $key }}" class="text-tpc-ink" @selected($category === $key)>{{ $label }}</option>
                        @endforeach
                    </select>
                    <button class="rounded-xl border-2 border-white bg-white px-6 py-2.5 text-sm font-bold text-tpc-primary hover:bg-tpc-accent hover:border-tpc-accent transition">
                        Filter
                    </button>
                </form>
            </div>
                ...
                @if(auth()->user()->is_super_admin)
                    <div id="bulk-bar"
                        ...
                        <p class="text-sm font-semibold text-gray-700">
                            <span id="bulk-count">0</span> feedback(s) selected
                        </p>
            ...
            <div class="space-y-4">
                    ...
                    <div class="relative group">
                        ...
                        <a href="{{ route('admin.feedback.show', $f) }}"
                           class="block bg-white rounded-2xl border border-gray-200 shadow-sm
                            ...
                            <div class="h-1 w-full {{ $f->is_read ? 'bg-gray-100' : 'bg-tpc-primary' }} group-hover:bg-tpc-accent transition-colors duration-300"></div>
                            ...
                            <div class="p-5">
                                <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <div class="flex flex-wrap items-center gap-2 mb-2">
                                            ...
                                            <span class="inline-flex items-center gap-0.5">
                                                @for($i = 1; $i <= 5; $i++)
                                                    <svg class="h-3.5 w-3.5 {{ $i <= $f->rating ? 'text-amber-400' : 'text-gray-200' }}" fill="currentColor" viewBox="0 0 20 20">
                                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.958a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.367 2.447a1 1 0 00-.363 1.118l1.287 3.957c.3.922-.755 1.688-1.539 1.118l-3.367-2.446a1 1 0 00-1.176 0l-3.367 2.446c-.784.57-1.838-.196-1.539-1.118l1.287-3.957a1 1 0 00-.363-1.118L2.063 9.385c-.783-.57-.38-1.81.588-1.81h4.163a1 1 0 00.95-.69l1.285-3.958z"/>
                                                    </svg>
                                                @endfor
                                            </span>
                                        ...
                                        <div class="flex items-center gap-2 mb-2">
                                            ...
                                            <p class="text-sm text-gray-500">
                                                <span class="font-semibold text-gray-700">{{ $f->name ?: 'Anonymous' }}</span>
                                                @if($f->email)
                                                    <span class="text-gray-400 ml-1">({{ $f->email }})</span>
                                                @endif
                                            </p>
                                        </div>
                                    ...
                                    <div class="shrink-0 flex flex-row sm:flex-col items-center sm:items-end gap-2 sm:gap-1">
                                        <p class="text-xs font-semibold text-gray-500">{{ $f->created_at->timezone('Asia/Manila')->format('M d, Y') }}</p>
                                        <p class="text-xs text-gray-400">{{ $f->created_at->timezone('Asia/Manila')->format('h:i A') }}</p>
                ...
                @empty
                    <div class="py-24 text-center border border-dashed border-gray-300 rounded-2xl bg-white">
                        <p class="text-lg font-semibold text-gray-300 mb-1">No feedback found</p>
