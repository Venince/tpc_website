        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-3">
                @if ($program->logo_path)
                    <img src="{{ asset('storage/' . $program->logo_path) }}"
                         class="h-12 w-12 rounded-xl bg-white object-contain p-1 shrink-0 shadow-neo-inset-sm"
                         alt="{{ $program->code }}">
                @else
                    <span class="h-12 w-12 rounded-xl bg-neo-bg shadow-neo-inset-sm flex items-center justify-center text-2xl shrink-0">🎓</span>
                @endif
                <div>
                    <h1 class="text-base font-semibold text-neo-ink leading-tight">{{ $program->name }}</h1>
            ...
            <div class="flex items-center gap-2 shrink-0">
                ...
                <a href="{{ route('admin.programs.edit', $program) }}"
                   class="inline-flex items-center gap-1.5 rounded-xl bg-tpc-primary px-4 py-2 text-xs font-semibold text-white
    ...
    <div class="grid gap-5 lg:grid-cols-2">
        ...
        <div class="rounded-2xl bg-neo-surface shadow-neo overflow-hidden">
            <div class="flex items-center justify-between px-5 py-3.5 border-b border-black/[0.06]">
                <p class="text-sm font-semibold text-neo-ink">People</p>
                <a href="{{ route('admin.programs.people.create', $program) }}"
                   class="inline-flex items-center gap-1 rounded-xl bg-tpc-primary px-3 py-1.5 text-xs font-semibold text-white
                          shadow-neo-sm transition hover:shadow-neo-hover active:shadow-neo-inset-sm">
                    <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
            ...
            @if ($program->people->isEmpty())
                <p class="px-5 py-10 text-center text-sm text-neo-ink/35">No people added yet.</p>
            @else
                <ul class="divide-y divide-black/[0.05]" id="sortable-people">
                    @foreach ($program->people as $person)
                        <li class="flex items-center gap-3 px-4 py-3 hover:bg-neo-bg/60 transition group"
                            data-id="{{ $person->id }}">
                            <span class="shrink-0 cursor-grab text-neo-ink/20 group-hover:text-tpc-primary/40 drag-handle select-none text-base">⠿</span>
                            @if ($person->photo_path)
                                <img src="{{ asset('storage/' . $person->photo_path) }}"
                                     class="h-9 w-9 rounded-full object-cover shrink-0 shadow-neo-inset-sm"
                                     alt="{{ $person->name }}">
                            @else
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-neo-ink truncate">{{ $person->name }}</p>
                ...
                <p class="px-5 py-2 text-[11px] text-neo-ink/30 border-t border-black/[0.05]">Drag rows to reorder.</p>
            @endif
        </div>
        ...
        <div class="rounded-2xl bg-neo-surface shadow-neo overflow-hidden">
            <div class="flex items-center justify-between px-5 py-3.5 border-b border-black/[0.06]">
                <p class="text-sm font-semibold text-neo-ink">Achievements</p>
            ...
            @if ($program->achievements->isEmpty())
                <p class="px-5 py-10 text-center text-sm text-neo-ink/35">No achievements added yet.</p>
            @else
                <ul class="divide-y divide-black/[0.05]" id="sortable-achievements">
                    @foreach ($program->achievements as $achievement)
                        <li class="flex items-start gap-3 px-4 py-3 hover:bg-neo-bg/60 transition group"
                            data-id="{{ $achievement->id }}">
                            <span class="mt-1 shrink-0 cursor-grab text-neo-ink/20 group-hover:text-tpc-primary/40 drag-handle select-none text-base">⠿</span>
                            @if ($achievement->photo_path)
                                <img src="{{ asset('storage/' . $achievement->photo_path) }}"
                                     class="h-11 w-11 rounded-xl object-cover shrink-0 shadow-neo-inset-sm" alt="">
                            @else
                                <span class="h-11 w-11 rounded-xl bg-neo-bg shadow-neo-inset-sm flex items-center justify-center text-lg shrink-0">🏆</span>
                            @endif
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-neo-ink truncate">{{ $achievement->title }}</p>
                                <p class="text-xs text-neo-ink/45">
