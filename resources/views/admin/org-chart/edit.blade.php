...
@section('title', 'Edit – ' . $orgChart->name)
...
@section('page_actions')
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-lg font-semibold text-neo-ink">Edit: {{ $orgChart->name }}</h1>
    </div>
@endsection

@section('content')

<form method="POST" action="{{ route('admin.org-chart.update', $orgChart) }}" enctype="multipart/form-data"
      class="max-w-xl mx-auto">
    @csrf
    @method('PATCH')

    <div class="rounded-2xl bg-neo-surface shadow-neo p-5 sm:p-6 space-y-5">
        @include('admin.org-chart._form', ['node' => $orgChart, 'parents' => $parents])
